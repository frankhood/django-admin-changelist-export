...
lint: ## check style with flake8
	flake8 django_admin_changelist_export tests
...
coverage: ## check code coverage quickly with the default Python
	coverage run --source django_admin_changelist_export runtests.py tests
	coverage report -m
	coverage html
	open htmlcov/index.html

docs: ## generate Sphinx HTML documentation, including API docs
	rm -f docs/django-admin-changelist-export.rst
	rm -f docs/modules.rst
	sphinx-apidoc -o docs/ django_admin_changelist_export
	$(MAKE) -C docs clean
	$(MAKE) -C docs html
